...
test.afterAll(async () => {
  ...
  if (firstFailure !== undefined) {
    throw new Error('Test case details cleanup failed.', { cause: firstFailure });
  }
});
...
test.describe.serial('Test Case Details UI', () => {
  ...
  test('Should navigate to Test Case Details and show history', async ({ page }) => {
    page.on('console', (msg) => console.log('NHẬT KÝ TRÌNH DUYỆT:', msg.text()));
    page.on('pageerror', (err) => console.log('LỖI TRÌNH DUYỆT:', err.message));
    page.on('response', (response) =>
      console.log('MẠNG TRÌNH DUYỆT:', response.url(), response.status()),
    );
    ...
    await expect(page.locator('text=Module:').locator('..')).toContainText('Auth');
  ...
  test('AbortController debounce prevents stale overwrite', async ({ page }) => {
    ...
    await page.goto(`http://localhost:${port}/#test-cases`);
    await page.waitForSelector('input[placeholder="Search Test Cases..."]');

    // Type 'slow' -> triggers request A after 500ms
    await page.fill('input[placeholder="Search Test Cases..."]', 'slow');
    // Wait 600ms to allow debounce
    await page.waitForTimeout(600);

    // Type 'fast' -> triggers request B after 500ms
    await page.fill('input[placeholder="Search Test Cases..."]', 'TC-UI-1');
    await page.waitForTimeout(600);
