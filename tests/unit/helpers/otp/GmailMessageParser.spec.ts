...
test('does not use an arbitrary number when the configured contract misses', () => {
  expect(
    GmailMessageParser.extractOtp('Reference 999999', 'Your verification code is {otp}'),
  ).toBeUndefined();
});

test('rejects values that are not exactly six digits', () => {
  expect(GmailMessageParser.extractOtp('Code AB-4821', 'Code {otp}')).toBeUndefined();
  expect(GmailMessageParser.extractOtp('Code 48210', 'Code {otp}')).toBeUndefined();
});
